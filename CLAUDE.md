...
**Adding a new GenUI template** (the most common UI task): the recipe is in `ARCHITECTURE.md` under "Template renderers", and a curl example for each existing template is in `README.md`. Summary: add to `SUPPORTED_TEMPLATES` in `genui.py`, optionally add a payload-shape validator and a context pre-compute helper, write `templates/genui/<name>.html` extending `_base.html`.
